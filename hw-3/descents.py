...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            return (1/l) * ((x @ self.w - y) ** 2).sum()
        elif self.loss_function is LossFunction.MAE:
            return (1/l) * (np.abs(x @ self.w - y)).sum()
        elif self.loss_function is LossFunction.LogCosh:
            return (1/l) * (np.log(np.cosh(x @ self.w - y))).sum()
        elif self.loss_function is LossFunction.Huber:
            condition = np.abs(x @ self.w - y) <= self.delta
            loss = np.where(condition, (x @ self.w - y)**2 / 2, self.delta * (np.abs(x @ self.w - y) - self.delta / 2))
            return np.mean(loss)
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        l = x.shape[0]
        if self.loss_function is LossFunction.MSE:
            return -(2 / l) * ((y - x @ self.w).T @ x)
        elif self.loss_function is LossFunction.MAE:
            return (1 / l) * (np.sign(x @ self.w - y).T @ x)
        elif self.loss_function is LossFunction.LogCosh:
            return (1 / l) * (np.tanh(x @ self.w - y).T @ x)
        elif self.loss_function is LossFunction.Huber:
            l = x.shape[0]
            condition = np.abs(x @ self.w - y) <= self.delta
            return (1/l) * np.where(condition, x @ self.w - y, self.delta * np.sign(x @ self.w - y)).T @ x
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        return super().calc_gradient(x[mask], y[mask])
...
class Adamax(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        step = - (self.lr() * self.m) / ((1 - self.beta_1 ** self.iteration) * self.u)
        self.w += step
        return step
